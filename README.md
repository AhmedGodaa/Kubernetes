...
- Abstract Component should take storage from actual physical storage like `hard drive`, `cloud provider`, `s3`
- PV are not namespaced

#### Persistent Volume Claim PVC

- Developers should configure their application yaml file to use persistent volume
- Applications has to claim that volume storage using PVC
- PVC are namespaced

> `📝 Note`
> PersistentVolumeClaim (PVC):\
> When a pod needs persistent storage, it makes a request via a PVC. The PVC specifies the storage requirements in terms
> of size, access mode, and optionally references a StorageClass.
...
metadata:
  name: k8s-test-pvc
spec:
  storageClassName: k8s-test-storage-class  # Reference to the "fast" StorageClass
...
- Create Persistence Volumes Dynamically when pvc claims it

> `📝 Note` StorageClass Definition:\
> You define a StorageClass in your Kubernetes cluster with specific parameters that describe the type of storage you
> want to provision. These parameters might include things like the provisioner (the storage provider), access modes,
> and
...
kind: StorageClass
metadata:
  name: k8s-test-storage-class
provisioner: kubernetes.io/aws-ebs  # The provisioner for AWS EBS volumes
parameters:
  type: gp2   # The EBS volume type (e.g., gp2 for General Purpose SSD)
  zone: us-west-1a  # The availability zone
...
**Prometheus Server**: Responsible for collecting and storing
time-series data, evaluating alerting rules, and serving the Prometheus Query Language (PromQL) to query and retrieve
data.

**Prometheus Data Model**: Is data stored and organized efficiently to facilitate
fast querying and analysis.

**Metrics Exporters**:
Prometheus relies on exporters, small applications or libraries that expose metrics in a format that
Prometheus can scrape. Examples like Node Exporter for system metrics, and exporters for databases, web servers, and
more.

**PromQL**: Prometheus Query Language (PromQL) is a powerful language used to query and manipulate time-series data. It
allows you to perform aggregations, transformations, and calculations on metrics to gain insights into system behavior
and performance.

> `📝` **Note**:
> PromQL can be replaced with grafana

**AlertManager**: Component responsible for handling alerts generated by the
